...
backgroundPosition:"top 10%", backgroundSize:"cover"
    ...
    <p className='mt-2 w-8/12' > {data.overview.slice(0,150)}...<Link to={`/${data.media_type}/details/${data.id}/trailer`} className='text-blue-400' >more</Link> </p>
    <div className='flex items-center gap-5 mt-2' >
    <Link to={`/${data.media_type}/details/${data.id}${data.release_date ? "/trailer" : ''}`} className=' mt-2 hover:scale-95 duration-100  bg-custompurple w-fit px-4 rounded-xl  py-2' > { data.release_date ? "Watch Trailer" :"Know More"  } </Link>
