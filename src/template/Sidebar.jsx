...
const Sidebar = () => {
  const feeds=[{name:"Trending", icon:<FaFire />, link:"/trending"}, {name:"Popular", icon:<SiGooglegemini />, link:"popular"}, {name:"Movies", icon:<BiSolidMoviePlay />, link:"movie"}, {name:"TV Shows", icon:<RiTvFill />, link:"tv"}, {name:"People", icon:<TiGroup />, link:"person"}]
  return (
    <>
    <div className='w-[20%] border-r-[0.8px] flex flex-col  border-zinc-400 h-full py-8 px-10 ' >
      <h1 className='text-white text-3xl font-bold' > Movie<span className='text-custompurple font-black italic text-3xl  ' >Park</span> </h1>
      ...
      <hr className='border-none h-[1px] my-2 bg-zinc-400'  />
         ...
         <Link to={'/about'} className='text-xl flex gap-1 justify-start items-center duration-300 text-zinc-400 px-5 py-2 rounded-md  hover:bg-custompurple hover:text-zinc-100 ' > <TbInfoHexagonFilled /> About Us </Link>
