...
import { FaFacebook, FaImdb, FaInstagram, FaRegArrowAltCircleLeft } from "react-icons/fa";
...
const PersonDetail = () => {
  ...
  return info ? (
    <div className="w-screen h-fit pb-10 bg-customblack px-4 md:px-[7%]">
      {/* Navbar */}
      ...
      <div className="w-full flex flex-col md:flex-row gap-10">
        ...
        <div className="w-full md:w-[20%] flex gap-y-1 flex-col">
          <img
            className="h-[50vh] w-full md:w-56 rounded-md object-cover"
            src={`https://image.tmdb.org/t/p/original${info.detail.profile_path}`}
            alt=""
          />
          <hr className="mt-2 border-none h-[2px] w-full md:w-56 bg-zinc-400 mb-1" />
          {/* Social Media Handles */}
          <div className="flex text-2xl text-zinc-200 w-full md:w-56 gap-x-4">
          ...
          <h3 className="text-xl text-gray-400 font-semibold">
            Known for - <span className="text-zinc-100">{info.detail.known_for_department}</span>
          </h3>
          <h3 className="text-xl text-gray-400 font-semibold">
            Birthday - <span className="text-zinc-100">{info.detail.birthday}</span>
          </h3>
          <h3 className="text-xl text-gray-400 font-semibold">
            Birth Place - <span className="text-zinc-100">{info.detail.place_of_birth}</span>
          </h3>
          <h3 className="text-xl text-gray-400 font-semibold">
            Gender - <span className="text-zinc-100">{info.detail.gender === 2 ? "Male" : "Female"}</span>
          </h3>
          <h3 className="text-lg text-gray-400 font-semibold">
            Also Known as - <span className="text-zinc-100">{info.detail.also_known_as.join(", ")}</span>
          </h3>
        </div>
        <div className="w-full md:w-[80%] flex flex-col gap-y-1">
          <h1 className="text-3xl md:text-5xl font-bold mb-1 text-zinc-100">{info.detail.name}</h1>
          ...
          <div className="w-full mt-5 list-disc overflow-x-hidden h-[40vh] overflow-y-auto shadow-lg shadow-[rgba(255,255,255,.5)]">
            {info[creditsKey]?.cast.map((card, index) => (
              <li key={index} className="hover:text-white text-zinc-400 hover:bg-zinc-800 p-5 rounded cursor-pointer">
                <Link to={`/${category}/details/${card.id}`}>
